...
func TestParseDefaultsToXGBoost(t *testing.T) {
	options := ParserOptions{ClusterID: "cluster-a", ScalerAddresses: map[string]struct{}{"replicasense.default.svc:6000": {}}, DefaultMaxReplicaCount: 100}
	result := Parse(validInput(), options)
	if len(result.Candidates) != 1 || result.Candidates[0].Spec == nil {
		t.Fatalf("valid input must produce one predictive workload: %#v", result)
	}
	if got, want := result.Candidates[0].Spec.Forecast.ModelEngine, "xgboost"; got != want {
		t.Fatalf("default model engine = %q, want %q", got, want)
	}
}
