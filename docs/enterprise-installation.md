...
If the `Chart.yaml` version is `0.2.0`, the tag must be `v0.2.0`.

```bash
git tag -a v0.2.0 -m "ReplicaSense 0.2.0"
...
- The chart runs components as non-root with a read-only root filesystem,
  ...
  PodDisruptionBudget.
- Native XGBoost is optional. Use the `0.2.0-xgboost` forecaster and trainer
  tags only when native model artifacts are intentionally enabled.
